...
import { useHoverSound } from "./hooks/useHoverSound.js";
...
const BootSequence = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    if (playPromise !== undefined) {
      playPromise.catch((e) =>
        console.warn("Audio blocked pending user interaction:", e),
      );
    }
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 md:p-12 font-mono text-sm md:text-base flex flex-col justify-end cursor-none relative overflow-hidden z-[300]">
      ...
      <div className="max-w-4xl w-full mx-auto space-y-2 mb-10 z-10">
        <h1 className="text-3xl md:text-4xl font-bold tracking-widest mb-8 text-[#dad4bb]">
          YoRHa OS [Version 11.42.3]
        </h1>

        {logs.map((log, i) => (
          <div
            key={i}
            className="flex justify-between w-full md:pr-8 animate-fade-in"
          >
            <span>&gt; {log.text}</span>
        ...
        <div className="mt-6 h-8">
          {progress === 100 && (
            <p className="text-xl tracking-widest animate-pulse text-[#dad4bb]">
              GLORY TO MANKIND.
            </p>
          )}
        </div>
...
const CustomCursor = ({ destructState }) => {
  ...
  return (
    ...
    >
      <div
        className={`absolute w-full h-[2px] opacity-70 ${destructState === "DESTRUCTING" ? "bg-red-600" : "bg-nier-accent"}`}
      ></div>
      <div
        className={`absolute h-full w-[2px] opacity-70 ${destructState === "DESTRUCTING" ? "bg-red-600" : "bg-nier-accent"}`}
      ></div>
      <div
        className={`w-2 h-2 border ${destructState === "DESTRUCTING" ? "bg-red-600 border-black" : "bg-nier-text border-nier-bg"}`}
      ></div>
    </div>
  );
};
...
const FloatingPod = ({ targetPos, destructState }) => {
  ...
  if (destructState === "DESTRUCTING" || destructState === "DESTROYED")
    return null;
  ...
  return (
    ...
    >
      <div className="relative animate-bounce">
        <div
          className={`w-6 h-8 border-[2px] border-black flex flex-col items-center justify-center transition-colors duration-300 ${isTargeting ? "bg-black" : "bg-[#dad4bb]"}`}
        >
          <div
            className={`w-2 h-2 rounded-full transition-colors duration-300 ${isTargeting ? "bg-red-600 animate-pulse" : "bg-black"}`}
          ></div>
        </div>
...
const SectionHeader = ({
  ...
  return (
    ...
    >
      <h2
        className={`text-2xl font-bold text-nier-text border-b-2 border-nier-border inline-block pr-4 pb-1 tracking-widest uppercase typewriter ${!recruiterMode ? "chromatic-hover" : ""}`}
      >
        {title}
      </h2>
      {!recruiterMode && (
        <div className="w-3 h-6 bg-nier-text animate-blink ml-1 mb-1"></div>
      )}
    </div>
  );
};
...
const NavButton = ({
  ...
  >
    <span
      className={`${!recruiterMode ? "group-hover:animate-glitch" : ""} inline-block`}
    >
      {label}
    </span>
  </button>
...
export default function App() {
  const initialSafeMode =
    ...
    localStorage.getItem("nierSafeMode") === "true";
  ...
  if (destructState === "DESTROYED" && !recruiterMode) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white font-mono cursor-none relative">
        ...
        <p className="text-2xl tracking-widest animate-pulse font-bold z-10">
          &gt; CONNECTION LOST
        </p>
        <button
  ...
  return (
    <div
      className={`min-h-screen text-nier-text p-4 md:p-8 relative overflow-hidden cursor-none ${destructState === "DESTRUCTING" ? "bg-black" : "bg-nier-bg"}`}
    >
        ...
        <div className="fixed inset-0 z-[200] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-nier-bg border-4 border-red-600 p-8 max-w-lg w-full animate-pulse shadow-[0_0_50px_rgba(220,38,38,0.5)] z-20 relative">
            ...
            <p className="text-nier-text font-bold mb-8 tracking-widest uppercase leading-relaxed">
              Initiating Black Box overload. Self-destruct sequence requested.
              All unsaved data and unit functionality will be permanently lost.
            </p>
      ...
      {!recruiterMode && (
        <FloatingPod targetPos={podPos} destructState={destructState} />
      )}

      <div
        className={`max-w-6xl mx-auto border-2 border-nier-border bg-[#dad4bb] relative z-10 shadow-2xl flex flex-col md:flex-row h-[85vh] animate-fade-in cursor-none ${destructState === "DESTRUCTING" && !recruiterMode ? "animate-extreme-glitch opacity-80" : ""}`}
      >
        ...
        <div className="w-full md:w-64 border-b-2 md:border-b-0 md:border-r-2 border-nier-border bg-[#dad4bb]/80 flex flex-col cursor-none">
          <div
            className={`p-6 border-b-2 border-nier-border relative overflow-hidden group cursor-none ${destructState === "DESTRUCTING" && !recruiterMode ? "bg-red-600 text-white" : ""}`}
          >
            {!recruiterMode && (
              <div className="absolute top-0 left-0 w-full h-1 bg-nier-text/20 group-hover:translate-y-24 transition-transform duration-1000"></div>
            )}
            <h1 className="text-xl font-bold tracking-widest cursor-none">
              YoRHa OS
            </h1>
        ...
        <div className="flex-1 p-8 overflow-y-auto bg-transparent relative cursor-none">
          <div
            className={`absolute top-0 right-0 w-8 h-8 border-t-4 border-r-4 m-4 cursor-none ${destructState === "DESTRUCTING" && !recruiterMode ? "border-red-600 animate-pulse" : "border-nier-text"}`}
          ></div>
          <div
            className={`absolute bottom-0 right-0 w-8 h-8 border-b-4 border-r-4 m-4 cursor-none ${destructState === "DESTRUCTING" && !recruiterMode ? "border-red-600 animate-pulse" : "border-nier-text"}`}
          ></div>

          {/* TAB 1: ABOUT (SYSTEM STATUS) */}
          {activeTab === "ABOUT" && (
            <div
              className="animate-fade-in opacity-0 cursor-none"
              style={{ animationDelay: recruiterMode ? "0ms" : "100ms" }}
            >
              <SectionHeader
                recruiterMode={recruiterMode}
                title="About :: Initialization"
                onHoverEnter={(e) => handleElementHover(e, -35, 5)}
                onHoverLeave={handleElementLeave}
              />
              <div
                className={`space-y-4 border border-nier-border p-6 bg-[#dad4bb]/90 transition-colors duration-300 cursor-none ${!recruiterMode ? "hover:border-nier-text" : ""}`}
              >
                <p className="font-bold cursor-none">
                  &gt;&gt; USER_ID: RONAK_CHAVHAN
                </p>
                <p className="cursor-none">
                  &gt;&gt; DESIGNATION: Computer Science Student | Software
                  Developer | Backend Enthusiast
                </p>
                <p className="mt-4 leading-relaxed cursor-none">
                  &gt;&gt; OBJECTIVE_DATA: Computer Science engineering student
                  specializing in full-stack software development, debugging,
                  and gamifying web interfaces. Strong programming foundation
                  with a passion for building immersive, interactive
                  applications, scalable design, and creative problem solving.
                </p>
                <div className="grid grid-cols-2 gap-4 mt-8 border-t border-nier-border pt-4 text-sm cursor-none">
                  <div className="cursor-none">
                    <strong className="cursor-none">LOCATION:</strong> Navi
                    Mumbai, India
                  </div>
                  <div className="cursor-none">
                    <strong className="cursor-none">STATUS:</strong>{" "}
                    <span
                      className={`text-green-600 font-bold cursor-none ${!recruiterMode ? "animate-pulse" : ""}`}
                    >
                      ONLINE
                    </span>
                  </div>
                  <div className="cursor-none">
                    <strong className="cursor-none">EDUCATION:</strong> B.Tech
                    CSE (ITM Skills Univ.)
                  </div>
                  <div className="cursor-none">
                    <strong className="cursor-none">SOFT_SKILLS:</strong>{" "}
          ...
          {activeTab === "SKILLS" && (
            ...
            >
              ...
              />
              <p
                className={`text-sm mb-6 opacity-80 inline-block cursor-none ${!recruiterMode ? "typewriter" : ""}`}
              >
                &gt;&gt; ALLOCATING MEMORY TO EQUIPPED SKILL CHIPS...
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 cursor-none">
                <div className="cursor-none">
                  <h3 className="font-bold mb-2 text-sm text-nier-accent cursor-none">
                    &gt; CORE_LANGUAGES
                  </h3>
                  <PluginChip
                    recruiterMode={recruiterMode}
                    ...
                    cost="4"
                    delay="200ms"
                    onHoverEnter={(e) => handleElementHover(e, -20, 5)}
                    onHoverLeave={handleElementLeave}
                  />
                  <PluginChip
                    recruiterMode={recruiterMode}
          ...
          {/* TAB 3: EXPERIENCE (WORK EXPERIENCE) */}
              ...
              <div className="space-y-6 cursor-none">
                ...
                >
                  <div className="flex justify-between items-start mb-2 cursor-none">
                    <h3 className="text-lg font-bold cursor-none">
                      &gt;&gt; Co-Founder | Student Food Venture
                    </h3>
                    <span className="text-xs border border-nier-text px-2 py-1">
                      CULINARY_VENTURE
                    </span>
                  </div>
                  <ul className="list-square list-inside text-sm space-y-2 mt-3 cursor-none">
                    <li className="cursor-none">
                      Launched and ran a small culinary venture with a partner,
                      owning marketing, supplier and partner communication,
                      profit-sharing, and financial tracking.
                    </li>
                    <li className="cursor-none">
                      Recovered the full initial capital investment and closed
          ...
          {/* TAB 4: PROJECTS (DATA ARCHIVES WITH HACKING MINIGAME) */}
          {activeTab === "PROJECTS" && (
            <div
              className="animate-fade-in opacity-0 cursor-none"
              style={{ animationDelay: recruiterMode ? "0ms" : "100ms" }}
            >
              <SectionHeader
                recruiterMode={recruiterMode}
                title="Projects :: Executed_Archives"
                onHoverEnter={(e) => handleElementHover(e, -35, 5)}
                onHoverLeave={handleElementLeave}
              />

              <div className="space-y-6 cursor-none">
                {/* Classified File Section with Hacking Minigame for Nexus-PR-Auditor */}
                <div
                  className={`border border-nier-border p-5 bg-[#dad4bb]/90 cursor-none`}
                >
                  <h3 className="text-lg font-bold mb-2 cursor-none">
                    &gt;&gt; CLASSIFIED_DATA :: NEXUS-PR-AUDITOR
                  </h3>

                  {/* Hide hacking interface completely in Simple Mode */}
                  {!isFileUnlocked && !isHacking && !recruiterMode && (
                    <div className="flex flex-col items-center justify-center py-6 border border-dashed border-red-600 bg-red-600/10 cursor-none">
                      <span className="text-red-600 font-bold mb-3 tracking-widest text-center cursor-none">
                        ACCESS DENIED. ENCRYPTION DETECTED.
                      </span>
                      <button
                        onClick={() => {
                          safePlayClick();
                        ...
                        onMouseEnter={(e) => handleElementHover(e, -35, 5)}
                        onMouseLeave={handleElementLeave}
                  ...
                  {/* Show decrypted data if unlocked OR if in Simple Mode */}
                  {(isFileUnlocked || recruiterMode) && (
                    <div className="animate-fade-in border-l-4 border-green-600 pl-4 py-2 mt-3 cursor-none">
                      {!recruiterMode && (
                        <span className="text-green-600 font-bold text-xs tracking-widest mb-2 block cursor-none">
                          DECRYPTION SUCCESSFUL
                        </span>
                      )}
                      <div className="flex justify-between items-start mb-2 cursor-none">
                        <h3 className="text-md font-bold cursor-none">
                          Nexus-PR-Auditor - AI-Driven Code Review Microservice
                        </h3>
                        <a
                          href="https://github.com/Ronak-uh"
                          target="_blank"
                          rel="noreferrer"
                          onMouseEnter={(e) => handleElementHover(e, -20, 5)}
                          onMouseLeave={handleElementLeave}
                          className={`text-xs border border-green-600 text-green-600 px-2 py-1 hover:bg-green-600 hover:text-black cursor-none ${!recruiterMode ? "hover:animate-glitch" : ""}`}
                        >
                          ACCESS_REPO
                        </a>
                      </div>
                      <ul className="list-square list-inside text-sm space-y-2 cursor-none">
                        <li className="cursor-none">
                          Developed a multi-agent swarm using the Google Gen AI
                          SDK to asynchronously audit algorithmic complexity and
                          scan for security vulnerabilities.
                        </li>
                        <li className="cursor-none">
                          Engineered a secure Node.js webhook pipeline
                          integrated with the GitHub REST API and MongoDB to
                          automate and track deterministic review verdicts.
                        </li>
                      </ul>
                    </div>
                  )}
                ...
                >
                  <div className="flex justify-between items-start mb-2 cursor-none">
                    <h3 className="text-lg font-bold cursor-none">
                      &gt;&gt; MapBorne - Indian Transport Accessibility
                    </h3>
                    <a
                      href="https://github.com/Ronak-uh/MapBorne.git"
                      target="_blank"
                      rel="noreferrer"
                      className={`text-xs border border-nier-text px-2 py-1 hover:bg-nier-text hover:text-nier-bg cursor-none ${!recruiterMode ? "hover:animate-glitch" : ""}`}
                    >
                      ACCESS_REPO
                    </a>
                  </div>
                  <ul
                    className={`list-square list-inside text-sm space-y-2 mt-3 cursor-none`}
                  >
                    <li className="cursor-none">
                      Interactive platform with time based route analysis.
                    </li>
                    <li className="cursor-none">
                      Improved route processing efficiency by 30% via optimized
                      API calls.
                    </li>
                    <li className="cursor-none">
                      Reduced data fetch delay by ~40% through Firebase sync
                      optimization.
                    </li>
                  </ul>
                </div>
                ...
                >
                  <div className="flex justify-between items-start mb-2 cursor-none">
                    <h3 className="text-lg font-bold cursor-none">
                      &gt;&gt; Unravel - Automated Blog Publishing
                    </h3>
                    <a
                      href="https://github.com/Ronak-uh/Unravel.git"
                      target="_blank"
                      rel="noreferrer"
                      className={`text-xs border border-nier-text px-2 py-1 hover:bg-nier-text hover:text-nier-bg cursor-none ${!recruiterMode ? "hover:animate-glitch" : ""}`}
                    >
                  ...
                  >
                    <li className="cursor-none">
                      Automated publishing pipeline with audit tracking.
                    </li>
                    <li className="cursor-none">
                      Reduced manual publishing effort by ~60% through workflow
                      automation.
                    </li>
                    <li className="cursor-none">
                      Lowered error rate by ~50% in repeated posting cycles.
                    </li>
                  </ul>
                </div>
              </div>
          ...
          {activeTab === "ACHIEVEMENTS" && (
            ...
            >
              <SectionHeader
                recruiterMode={recruiterMode}
                title="Achievements :: Combat_Logs"
                onHoverEnter={(e) => handleElementHover(e, -35, 5)}
                onHoverLeave={handleElementLeave}
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 cursor-none">
                <div
                  onMouseEnter={(e) => handleElementHover(e, -20, 5)}
                  onMouseLeave={handleElementLeave}
                  className={`border-l-4 border-nier-accent pl-4 py-2 bg-[#dad4bb]/80 transition-colors animate-fade-in opacity-0 cursor-none ${!recruiterMode ? "hover:bg-nier-bg" : ""}`}
                  style={{ animationDelay: recruiterMode ? "0ms" : "200ms" }}
                >
                  <h4 className="font-bold uppercase tracking-wider cursor-none">
                    Hackathon Participant
                  </h4>
                  <p className="text-sm mt-1 cursor-none">
                    Competed in multiple hackathons, building deployable
                    solutions under 24-72 hour time constraints.
                  </p>
                </div>
                ...
                <div
                  ...
                  onMouseLeave={handleElementLeave}
                  className={`border-l-4 border-nier-accent pl-4 py-2 bg-[#dad4bb]/80 transition-colors animate-fade-in opacity-0 cursor-none ${!recruiterMode ? "hover:bg-nier-bg" : ""}`}
                  style={{ animationDelay: recruiterMode ? "0ms" : "300ms" }}
                >
                  <h4 className="font-bold uppercase tracking-wider cursor-none">
                    Efficiency Expert
                  </h4>
                  <p className="text-sm mt-1 cursor-none">
                    Optimized API calls in MapBorne improving efficiency by 30%.
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="text-sm mt-1 cursor-none">
                    Reduced manual efforts by 60% in Unravel via workflow
                    automation.
                  </p>
                </div>

                <div
                  className={`border-l-4 border-nier-border pl-4 py-2 bg-[#dad4bb]/50 opacity-60 cursor-none`}
                  style={{ animationDelay: recruiterMode ? "0ms" : "500ms" }}
                >
                  <h4 className="font-bold uppercase tracking-wider cursor-none">
                    Certifications
                  </h4>
                  <p className="text-sm mt-1 cursor-none">
                    [ DATA NOT FOUND / AWAITING CLEARANCE ]
                  </p>
                </div>
              </div>
            </div>
          ...
          {activeTab === "CONTACT" && (
            <div
              className="animate-fade-in opacity-0 flex flex-col items-center justify-center h-full cursor-none"
              style={{ animationDelay: recruiterMode ? "0ms" : "100ms" }}
            >
              <SectionHeader
                recruiterMode={recruiterMode}
                title="Contact :: Establish_Link"
                onHoverEnter={(e) => handleElementHover(e, -35, 5)}
                onHoverLeave={handleElementLeave}
              />

              <div
                className={`w-full max-w-md border-2 border-nier-border bg-[#dad4bb]/90 p-8 mt-4 text-center transform transition-transform duration-300 cursor-none ${!recruiterMode ? "hover:scale-105" : ""}`}
              >
                <div
                  className={`w-16 h-16 rounded-full border-2 border-nier-text mx-auto mb-6 flex items-center justify-center cursor-none ${!recruiterMode ? "animate-pulse" : ""}`}
                >
                ...
                <div className="space-y-4 font-mono cursor-none">
                  <a
                    href="mailto:ronakchavhan89@gmail.com"
                    onMouseEnter={(e) => handleElementHover(e, -30, 5)}
                    onMouseLeave={handleElementLeave}
                    className={`block w-full border border-nier-border py-2 transition-colors group cursor-none ${!recruiterMode ? "hover:bg-nier-text hover:text-nier-bg" : ""}`}
                  >
                    <span
                      className={`inline-block cursor-none ${!recruiterMode ? "group-hover:animate-glitch" : ""}`}
                    >
                      ronakchavhan89@gmail.com
                    </span>
                  </a>

                  <a
                    href="https://linkedin.com/in/ronak-chavhan-b3343a32b/"
                    target="_blank"
                    rel="noreferrer"
                    onMouseEnter={(e) => handleElementHover(e, -30, 5)}
                    onMouseLeave={handleElementLeave}
                  ...
                  <div
                    className={`mt-8 pt-6 border-t border-nier-border cursor-none`}
                  >
                    <a
          ...
          {activeTab === "CLASSIFIED_LOGS" && (
            ...
            >
              <SectionHeader
                recruiterMode={recruiterMode}
                title="Classified_Logs :: Personal_Data"
                onHoverEnter={(e) => handleElementHover(e, -35, 5)}
                onHoverLeave={handleElementLeave}
              />
              ...
              <div className="space-y-6 cursor-none">
                ...
                >
                  <h3
                    className={`text-lg font-bold mb-4 text-red-600 cursor-none ${!recruiterMode ? "animate-pulse" : ""}`}
                  >
                    &gt;&gt; WARNING: UNAUTHORIZED ACCESS DETECTED
                  </h3>
                  <p className="mb-4 opacity-80 font-mono text-sm cursor-none">
                    Decrypted personal logs for Unit R.CHAVHAN...
                  </p>

                  <ul
                    className={`list-square list-inside text-sm space-y-4 cursor-none`}
                  >
                    <li className="cursor-none">
                      <strong className="cursor-none">
                        HARDWARE_MODIFICATION:
                      </strong>{" "}
                      Highly proficient in local hardware maintenance, including
                      DIY laptop LCD replacements, custom thermal paste
                      applications, and tracking the latest RTX 50-series
                      performance metrics.
                    </li>
                    <li className="cursor-none">
                      <strong className="cursor-none">
                      ...
                      Actively engaged in high-difficulty combat scenarios.
                      Current favorite simulations include{" "}
                      <em className="cursor-none">
                        Clair Obscur: Expedition 33
                      </em>{" "}
                      and <em className="cursor-none">Lies of P</em>.
                    </li>
                    <li className="cursor-none">
                      <strong className="cursor-none">DATA_CONSUMPTION:</strong>{" "}
                      Archiving literary records. Highest-rated archive is{" "}
                      <em className="cursor-none">
                        The Beginning After The End
                      </em>{" "}
                      by TurtleMe. Currently processing{" "}
                      <em className="cursor-none">
                        What Do You Wish For With Muddy Eyes: Record of Highserk
                        War
                      </em>{" "}
                      by Torutonen.
                    </li>
        ...
        <div className="absolute -bottom-8 left-0 w-full text-center text-xs opacity-40 cursor-none z-20">
          UI inspired by Nier: Automata. Background Music © Square Enix.
